...
for node in node_config.items():
  ...
  node_config[node[0]]['battery_status'] = node_config[node[0]]['node'].addProperty("battery-status", "Battery Status")
  node_config[node[0]]['battery_level'] = node_config[node[0]]['node'].addProperty("battery-level", "Battery Level")
